-- =========================================================
-- WQMS Node-Only Schema (NO USERS)
-- Forwarder -> HiveMQ -> Supabase -> UI
-- Run this entire script in Supabase Dashboard → SQL Editor
-- Best on a fresh project; if tables exist, constraints may error on re-run.
-- =========================================================
...
-- =========================================================
-- 1) NODES (with soft-delete: deactivated_at)
-- =========================================================
...
comment on column public.nodes.deactivated_at is
  'Updated via UI. When set, node is deactivated (soft-deleted); status becomes deactivated. Null = active.';

create index if not exists idx_nodes_status
  on public.nodes(status);

create index if not exists idx_nodes_last_seen
  on public.nodes(last_seen_at desc);
...
-- Keep status in sync with deactivated_at (UI updates deactivated_at; this sets status)
...
-- =========================================================
-- 2) SENSOR READINGS (INGESTION TABLE)
-- =========================================================
...
-- =========================================================
-- 3) DAILY SUMMARIES (LOW / AVG / HIGH)
-- =========================================================
create table if not exists public.daily_summaries (
  ...
  constraint uq_daily_summaries_node_date
    unique (node_id, date)
);
...
-- =========================================================
-- 4) THRESHOLDS (GLOBAL OR PER NODE)
-- =========================================================
create table if not exists public.thresholds (
  ...
  constraint thresholds_scope_check
    check (scope in ('global','node')),

  constraint thresholds_scope_node_rule
    check (
      (scope = 'global' and node_id is null) or
      (scope = 'node' and node_id is not null)
    ),

  constraint uq_thresholds_scope_node
...
create table if not exists public.calibration_settings (
  ...
  constraint calibration_scope_check
    check (scope in ('global','node')),

  constraint calibration_scope_node_rule
    check (
      (scope = 'global' and node_id is null) or
      (scope = 'node' and node_id is not null)
    ),

  constraint uq_calibration_scope_node
    unique (scope, node_id)
);
...
select 'global'
where not exists (
  select 1 from public.calibration_settings
  where scope = 'global' and node_id is null
);
...
create table if not exists public.alerts (
  ...
  constraint alerts_type_check
    check (type in ('threshold_breach','node_offline','sensor_test_failed','system')),
...
create index if not exists idx_alerts_node_time
  on public.alerts(node_id, triggered_at desc);

-- =========================================================
-- 7) NOTIFICATIONS (SYSTEM-WIDE)
-- =========================================================
...
-- =========================================================
-- TRIGGER: UPDATE NODE STATUS WHEN DATA ARRIVES
-- =========================================================
...
cross join lateral (
  ...
  where (scope = 'node' and node_id = r.node_id)
     or (scope = 'global' and node_id is null)
